...
| 2026-09-16 | Fix mapa: agenda sin sede | ✅ | Leaflet real lanzaba `Invalid LatLng (undefined, undefined)` con eventos sin sede (10/11 futuros) → guard en `agregarCapa` + filtro en `cargarAgenda` · commit `7748458` · deploy OK, live 200 |
| 2026-09-16 | G: Solo Región del Maule | ✅ | Humedal de Putú corregido (-35.3600,-72.5500 → **-35.2131,-72.2839**, estaba en el océano; desc actualizada a Putú-Huenchullami) · **Rocas de Constitución — Piedra de la Iglesia** (-35.3295,-72.4342) agregada como punto costero legítimo · polígono ADM1 vendored (`static/data/maule.geojson`, BCN/geoBoundaries CC BY 3.0 IGO) · `mapa.js`: fitBounds sobre bbox real + `setMinZoom`/`setMaxBounds` (navegación restringida al Maule) + máscara translúcida papel que oculta el exterior (hueco evenodd con orientación validada) · prueba point-in-polygon: 61 puntos, 0 fuera · build OK |
| 2026-09-16 | H: Fix clima Maulino | ✅ | Widget climático roto: `scripts.html` envolvía `.Site.Params.cities` con `slice()` → `window.DM.cities=[[{...}]]`, `weather.js` usaba `cities[0]` (arreglo) → `forecast(undefined,undefined)` → Open-Meteo 400 → "el cielo maulino no respondió". Quitado `slice` de cities/categories (apps planos; de paso arregla `catMeta` de la búsqueda) + `weather.js` normaliza y valida lat/lon antes del fetch · smoke: 6/6 ciudades 200 · build OK |
| 2026-09-20 | I: Agua animada en el hero | ✅ | Hero con fondo de agua: shader WebGL procedural propio (`static/js/water.js`, sin dependencias, ~7KB) tintado con la paleta "Río Maule" (olas viajeras + rayo de sol + caustics + viñeta). Rollout opción 1: velo + texto encima. Full-bleed con el truco de los ríos; pausa con IntersectionObserver/visibility; paralaje sutil de mouse; respeta prefers-reduced-motion; fallback gradiente CSS sin WebGL. Candidato a revisión: si no conforme → opción 2 (agua en todo el hero). |
| 2026-09-25 | I2: Opción 2 (agua en todo el hero) + shader v2 | ✅ | Hero a toda pantalla (`min-height: 92svh`), marquee pegado a la base, river--top y colchón de papel quitados en home, shader v2 con FBM/espuma/destellos · commits `40aeb25` opción 1, `94fc896` opción 2 |
| 2026-09-25 | J: Agua realista three.js (exploración) | ✅ | Probado **three.js `Water`** (ejemplo growplan/demo): horizonte + río calmo, vendores `three.module.min.js` + shim `three.core.min.js` (MIME fix) + `Water.js` addon; `new Water(` por import por nombre. **Rechazado por el usuario**: ni la composición horizonte ni el color/brillo del agua convencieron. Commits `83f24b8`, `675ac96`, `a744deb`, `1679ba5` |
| 2026-09-25 | K: Agua realista PlayCanvas (Water script oficial) | ✅ | Motor PlayCanvas ESM vendored (`playcanvas.min.js` 631KB gz) · Water script oficial de marklundin (`pc-water.js` + `pc-planar-renderer.js`, import interno `.mjs`→`.js` reescrito) · `hero-agua.js` reescrita: `pc.Application`, dome cielo procedural, glow de sol, luz direccional, agua Gerstner suave con `shallowColor` luminoso + arena (atiende el feedback de color/brillo), reflexión planar, normal map procedimental · DPR 1 táctil / 1.5 escritorio · importmap solo en home, interiores sin motor · vendor `three.*` (3 archivos) eliminados · fallbacks idénticos (sin WebGL2 / reduced-motion → gradiente) |